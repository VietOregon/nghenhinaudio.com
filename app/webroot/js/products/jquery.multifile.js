...
if (window.jQuery)(function ($) {
	...
	*/
	$.fn.MultiFile.options = { //$.extend($.fn.MultiFile, { options: {
		accept: 'jpg, png, gif, jpeg', // accepted file extensions
		...
		// previews
		preview: false,
		previewCss: 'max-width:250px;max-height:200px;',

		// STRING: collection lets you show messages in different languages
		STRING: {
			remove: 'Xóa',
			denied: 'Bạn không thể chọn một file $ext.',
			file: '$file',
			selected: 'Đã chọn: $file',
			duplicate: 'File đã được chọn: $file',
			toomuch: 'Số file được chọn quá nhiều ($size)',
			toomany: 'Too many files selected (max: $max)',
			toobig: '$file có dung lượng quá lớn (max $size)'
		},
